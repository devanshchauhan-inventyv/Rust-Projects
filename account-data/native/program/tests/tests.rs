...
fn test_account_data() {
    ...
    svm.add_program_from_file(program_id.pubkey(), "../../../target/deploy/account_data.so").unwrap();

    let instruction_data = borsh::to_vec(&AddressInfo { name: "Devansh Chauhan".to_string(), house_number: 7, street: "Sitaram-baug".to_string(), city: "Surendranagar".to_string() }).unwrap();

    let initialize_instruction = Instruction::new_with_bytes(program_id.pubkey(), &instruction_data, vec![AccountMeta::new(address_info_account.pubkey(), true), AccountMeta::new(payer.pubkey(), true), AccountMeta::new_readonly(solana_system_interface::program::ID, false)]);

    let message = Message::new(&[initialize_instruction], Some(&payer.pubkey()));

    let transaction = Transaction::new(&[&address_info_account, &payer], message, svm.latest_blockhash());

    let txn_result = svm.send_transaction(transaction);

    assert!(txn_result.is_ok(), "Address Account successfully initialized");
